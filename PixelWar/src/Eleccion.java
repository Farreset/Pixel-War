...
public class Eleccion extends JPanel implements ActionListener{
	...
	String fotos[] = {null,"Stickman","Mario", "Link", "Pikachu","Donkey Kong","Sonic","Creeper","Goku","Ratchet & Clank","Sub-Zero"};//todos los nombre de los equipos para luego passarlos en las funciones
	String s;
	private Font fuente;
	int cantidades;
	static ArrayList<Equipo> equipos = new ArrayList<Equipo>();//almacen de eqpos
...
//assets y diseño
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(cantidades == 3) {
			if(e.getSource()==start) {
				String s1 =	s = (String) pers1.getSelectedItem();//coge el nombre y lo analiza
				String s2 =	s = (String) pers2.getSelectedItem();
				String s3 =	s = (String) pers3.getSelectedItem();
				//comprovar nombres si son iguales o si no hay nada
				if(s1 == null || s2 == null || s3 == null) {//haya que elegir un tipo
					JOptionPane.showInternalMessageDialog(null, "ELIGE TIPO DE EQUIPO", "Error de tipo de equipo", 0);
				}
				//para que no esten vacios
				...
				else {//pasas el nombre a la funcion batalla
	...
	private void creacionpersonajes(int cantidad, String tipos[], String nom[]) {
		...
		for (int i = 0; i < cantidad+1; i++) {
			...
			if (i == 1) {
				...
				equipos.add(equipo1);//añade el equipo al array passandole el tipo y el nombre
	...
	public void plantillas(int cantidad) {
		...
		//para que se muestree la foto del tipo elegido
		for(int i = 0; i < cantidad-1; i++) {
			if (i == 1) {
				...
				pers1 = new JComboBox<String>(fotos);//listado de tipos que hay
				...
				pers1.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent ae) {
						...
						s = (String) pers1.getSelectedItem();//string del pesonaje (nombre del tipo)
						imgEq.setIcon(new ImageIcon("assets/caras/"+s +".png"));//ruta completa del tipo
						imgEq.setBounds(430, 48, 96, 93);
					}
				});

				imgEq = new JLabel(new ImageIcon());//se añade la imagen
				add(imgEq);
